...
def run_model(site_name):
    ...
    cmd='python zfx_mode.py' +' --data_path '+site_name+'.csv'
    print(cmd)
...
def save_data(now_time, site_name):
    ...
    with open(".pred_file_path", "r") as f:
        pred_file_path = f.read()

    loaddata = np.load(
        './results/{}/real_prediction_{}.npy'.format(
            pred_file_path,site_name))
    # zfx update end
    data_log = pd.DataFrame(loaddata[:, :, 0], index=['prediction_rainfall'])

    for i in range(0,data_log.shape[1]):
        if isnan(data_log[i].values):
            data_log.iloc[0,i]=0
...
# schedule.every().hour.at('00:00').do(job(site_name))  # 在每小时的00分00秒开始，定时任务job
# schedule.every().hour.at('30:00').do(job(site_name))  # 在每小时的00分00秒开始，定时任务job
...
# 脚本调用示例：/usr/bin/python3 /home/ubuntu/GPU_model_predict/timer_run_informer_task.py 站点名称 当前时间
args = sys.argv
if len(args) < 3:
    print("参数不合法")
else:
  print("站点名称:", args[1])
  print("当前时间:", args[2])
# site_name='B11',time.time()
job(args[1],float(args[2]))
# job('B06',time.time())
