...
def get_data(now_time, history_time, site_name):
    ...
    data_csv.to_csv('./tmp/{}.csv'.format(site_name),mode='a', index=False)
...
def job(site_name, now_time):  # 定时任务
    ...
    # site_name = "B19"  # 读取xx站点的数据库
    print(time.strftime("%Y_%m_%d %H:%M:%S", time.localtime(history_time)))  # 当前时间
    flag = get_data(now_time, history_time, site_name)
    if flag == 1:
        # run_model()
        # save_data(now_time, site_name)
        print('Successfully Done')
        return time.localtime(history_time)
    else:
        print('No valid data')
        return time.localtime(history_time)
...
def get_args():
    ...
    parser.add_argument('--start', type=str, required=True, help='开始时间：如2021-01-01')
    parser.add_argument('--end', type=str,required=True, help='结束时间：如2021-08-21')
...
if __name__ == '__main__':
    ...
    bjt = datetime.timezone(td)
    start_time = datetime.datetime.strptime(arge.start, "%Y-%m-%d").replace(tzinfo=bjt)
    end_time = datetime.datetime.strptime(arge.end, "%Y-%m-%d").replace(tzinfo=bjt)
    for i in range(0, 1000):
        x = start_time + datetime.timedelta(hours=15*i)
        #print(x.timestamp())
        time_object =  job(arge.name, x.timestamp())  # 传入时间
        time_object = datetime.datetime.fromtimestamp(calendar.timegm(time_object), bjt)
        if time_object >= end_time:
            break
